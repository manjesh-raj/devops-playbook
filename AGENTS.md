...
- **Native theme-aware diagrams** (no raster screenshots): build them with inline SVG or CSS grid and drive every color off tokens so they flip with the theme. Two reusable patterns already exist in the OTel `otel-tml` section: `.otel-seq*` (an SVG sequence diagram - lifelines, activation bars, request/response arrows via `<marker>`, colors set inline as `fill:var(--blue)` etc.) and `.otel-wf*` (a Jaeger-style trace waterfall in CSS grid - bars positioned with inline `left`/`width` percentages, indented per depth). Wrap any wide SVG in an `overflow-x:auto` scroller with a `min-width` so it scrolls internally on mobile instead of overflowing the page.
