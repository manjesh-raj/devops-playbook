...
- **Handwritten "notebook" module (`nb-*`)** is a deliberately distinct, self-contained look used by the Kubernetes Notes concept (a pilot). It is isolated from every other section - do not restyle other concepts to match it, and do not fold notebook CSS into the shared component kit.
  - It is ONE `<section id="nb-k8s-cover" class="section nb-section nb-book-section">` whose children are `.nb-leaf` panels (cover, 5 pages, an end/back-cover leaf) inside `.nb-book > .nb-stage`. Each leaf holds a `.nb-sheet` (`.nb-cover` or `.nb-page`) with a CSS spiral binding (`.nb-rings` capsules over a gutter `::after`), a red margin line (`::before`), and ruled paper (a `repeating-linear-gradient` background).
  - **Page-turn "book" mode is progressive enhancement.** `initNbBook()` in `script.js` adds `.nb-ready`, then shows one leaf at a time in a fixed reading frame and animates 3D `rotateY` page turns (book-open swing on the cover, book-close swing onto the end leaf). Without JS the leaves fall back to a normal vertical stack that scrolls, so **content is never gated on the animation**. Navigation is Prev/Next buttons + dog-ear corners + keyboard Left/Right (only while the book fills the viewport) + touch swipe. Each leaf scrolls INTERNALLY (`overflow-y:auto`); only movement BETWEEN leaves is a turn.
  - **Scroll-spy/breadcrumb/deep-links for the book:** leaf ids equal the sidebar `data-section` ids, so `#nb-k8s-pods` (etc.) deep-links open the book directly on that leaf (fast-forwarding the open swing). The controller keeps `window.nbActivePageId` in sync and `setActiveSection()` redirects the section id `nb-k8s-cover` to that leaf so the breadcrumb/nav track the current page. Nav-link clicks route through `window.nbBookGoto()` (installed by the controller, called from `scrollTo_`) instead of scrolling to a hidden leaf.
